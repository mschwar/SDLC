...
> [!summary] Note Use
> Canonical blueprint note for an agent-first software delivery workflow.
> Use the outline pane to jump by layer, collapse the reference-heavy sections when you only need the workflow, and treat **Optional Advanced Extensions** as scale-up material rather than baseline requirements.
...
- In Obsidian, use the outline pane and fold `##` sections to switch between handbook mode and lookup mode.
...
> [!important] Quality Before Throughput
> Research shows AI agents increase PR volume by ~98% but do not automatically improve delivery quality.
> More PRs do not mean better outcomes.
> Quality checks are more important than velocity metrics. Optimize for pass rate first, throughput second.
...
> [!warning] Failure Modes To Watch
