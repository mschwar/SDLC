...
class ReleaseGatewayHandler(BaseHTTPRequestHandler):
...
def make_server(host: str = "127.0.0.1", port: int = 8000) -> ThreadingHTTPServer:
    return ThreadingHTTPServer((host, port), ReleaseGatewayHandler)


def run_server(host: str = "127.0.0.1", port: int = 8000) -> None:
    server = make_server(host=host, port=port)
    print(f"Serving release gateway on http://{host}:{server.server_port}")
    try:
